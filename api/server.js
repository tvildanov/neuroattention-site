...
app.post('/api/run-migrations', async (req, res) => {
  try {
    ...
    // 062-064 are reserved for the parallel unified-events (P6) + external-calendar
    // (P7) work. This block is additive + idempotent and does NOT depend on them:
    // it CREATEs the shared `oauth_tokens` table IF NOT EXISTS (identical schema to
    // P7's mig063 — whichever lands first wins, the other is a no-op) plus its own
    // `health_metrics` + `wearable_sync_log` tables. Gated feature is inert until
    // WEARABLES_ENABLED=true. See WEARABLES-OAUTH-REPORT.md.
    ...
    res.json({ ok: true, message: 'Migrations 003-065 applied successfully', mig039, mig040, mig041, mig042, mig043, mig044, mig045, mig046, mig047, mig048, mig049, mig051, mig052, mig053, mig054, mig055, mig056, mig057, mig058, mig059, mig060, mig061, mig065 });
  } catch (err) {
